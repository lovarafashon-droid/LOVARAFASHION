...
const CategoryApp = {
  ...
  translations: {
    en: {
      ...
      pageTitleAccessories: "LOVARA – Accessories", pageTitleSets: "LOVARA – Sweatshirts", pageTitleSuits: "LOVARA – Suits", pageTitlePajamas: "LOVARA – Pajamas", pageTitleLingerie: "LOVARA – Lingerie",
      ...
      catDresses: "Dresses", catTops: "Tops", catPants: "Pants", catAccessories: "Accessories", catSets: "Sweatshirts", catSuits: "Suits", catPajamas: "Pajamas", catLingerie: "Lingerie",
      ...
      chatLabel: "Chat", whatsappSupport: "WhatsApp Support", whatsappSupportDesc: "Chat with us on WhatsApp",
      ...
      footerPants: "Pants", footerAccessories: "Accessories", footerLingerie: "Lingerie", footerSets: "Sweatshirts", footerSuits: "Suits", footerPajamas: "Pajamas", footerSuits: "Suits", footerPajamas: "Pajamas",
    ...
    ar: {
      ...
      pageTitleAccessories: "LOVARA – إكسسوارات", pageTitleSets: "LOVARA – سويت شرت", pageTitleSuits: "LOVARA – سوتات", pageTitlePajamas: "LOVARA – بيجامات", pageTitleLingerie: "LOVARA – ملابس داخلية",
      ...
      catDresses: "فساتين", catTops: "توبات", catPants: "بناطيل", catAccessories: "إكسسوارات", catSets: "سويت شرت", catSuits: "سوتات", catPajamas: "بيجامات", catLingerie: "ملابس داخلية",
      ...
      chatLabel: "شات", whatsappSupport: "دعم واتساب", whatsappSupportDesc: "تواصلي معانا على واتساب",
      ...
      footerPants: "بناطيل", footerAccessories: "إكسسوارات", footerLingerie: "ملابس داخلية", footerSets: "سويت شرت", footerSuits: "سوتات", footerPajamas: "بيجامات", footerSuits: "سوتات", footerPajamas: "بيجامات",
  ...
  async handleGoogleAuth(modalId) {
    try {
      ...
      try {
        ...
        if (!message.includes('missing initial state')) throw firstError;
        try {
          Object.keys(sessionStorage).filter(key => /firebase|auth|redirect/i.test(key)).forEach(key => sessionStorage.removeItem(key));
        } catch (_) {}
        result = await auth.signInWithPopup(provider);
      }
  ...
  addToCart(product, size, color, quantity = 1, pricingUnit = 'piece') {
    ...
    if (existing) {
      existing.quantity = ((existing.qty ?? existing.quantity) || 1) + selectedQuantity;
      existing.qty = existing.quantity;
      existing.quantityDiscountEnabled = existing.quantityDiscountEnabled === true || selectedQuantity > 1;
    } else {
      this.cart.push({
        id: product.id,
        name: product.name,
        ...
        quantityDiscountEnabled: selectedQuantity > 1
      });
    }
    this.saveCart();
    this.updateCartCount();
  ...
  openProductDetail(productId) {
    ...
    const updateImage = () => {
      ...
      image.src = this.previewImages[this.previewImageIndex] || 'https://via.placeholder.com/400x500?text=LOVARA';
      image.alt = product.name || 'Product';
      count.textContent = `${this.previewImageIndex + 1}/${Math.max(this.previewImages.length, 1)}`;
      modal.querySelector('.category-preview-prev').disabled = this.previewImages.length < 2;
